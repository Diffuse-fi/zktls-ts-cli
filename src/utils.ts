...
import P from "pino";
import { ClaimTunnelResponse } from '@diffusefi/fetch-proof-server/lib/proto/api';
const logger = P();
...
export async function fetchAppById(appId: string): Promise<string> {
  ...
  try {
    const response = await fetch(`${APP_BACKEND_URL}/api/fetch-proof-client/sdk/${appId}`);
...
export async function sendLogs(
  {
    ...
    try {
      ...
      const url = `${LOGS_BACKEND_URL}/api/business-logs/fetch-proof-client`
